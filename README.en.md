...
- **Mobile (planned)**: currently **React Native Web** (sharing **the same `RelayChatBackend`** engine as desktop, so features are highly aligned); **an Android test APK does ship** (see [Releases](https://github.com/VaalRL/Cinderous/releases), a Capacitor shell, since v0.0.14); 🔴 it is currently **signed with a debug certificate and is debuggable** — anyone who can attach USB debugging can read process memory, and the private key is necessarily in there during a session, so do not use it for conversations you actually care about (ADR-0335, see [`docs/SECURITY.md`](./docs/SECURITY.md)). **iOS has not shipped** (it also needs a macOS build). Native notifications and background push remain to be wired.
