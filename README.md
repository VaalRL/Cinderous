...
**🔥 官方網站：<https://vaalrl.github.io/Cinderous/>** ｜ **🌐 在瀏覽器開啟（網頁版）：<https://cinderous.cinderous1.workers.dev>** ｜ **💻 下載桌面版：[GitHub Releases](https://github.com/VaalRL/Cinderous/releases)**
...
- **端到端加密**：訊息以 Nostr **NIP-17/44/59 Gift Wrap** 加密後才離開裝置——中繼站看不到內容，寄件者也以一次性金鑰隱藏（訊息層社交圖譜不可見；在線訂閱仍會讓 relay 得知你的聯絡人集合，見 `docs/SECURITY.md` 殘餘）。
...
| 引擎 | 技術 | 定位 |
| --- | --- | --- |
| **引擎 A：狀態與信令** | Nostr 協議（Cloudflare Workers；SQLite-backed Durable Objects） | 離線留言暫存、線上狀態廣播、WebRTC 初始 SDP 信令交換 |
...
- **離線留言**暫存（NIP-17/59 Gift Wrap ＋ NIP-40 過期；`message-store` 邏輯已實作並測試——每收件人配額、以 `#p` 為索引。持久化：Cloudflare 版用 SQLite-backed Durable Object（`new_sqlite_classes`）、Node 自架版用內建 SQL）。
