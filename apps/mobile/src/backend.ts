...
import { type AppStorage, type ChatBackend, LocalStorage, RelayChatBackend, webSocketConnector } from "@cinder/engine";
 ...
 * `store` 可由外部提供（ADR-0094：讓 App 持有同一份儲存以套用保留上限/導出）。
 */
export function createRelayChat(identity: MobileIdentity, relayUrl: string, store?: AppStorage): ChatBackend {
  return new RelayChatBackend(store ?? new LocalStorage(identity.pubkey), webSocketConnector(relayUrl), identity.name, {
    relayUrl,
...
export function createBackend(identity: MobileIdentity, relayUrl: string | null, store?: AppStorage): ChatBackend {
  return relayUrl ? createRelayChat(identity, relayUrl, store) : createDemoChat(identity.name);
}
