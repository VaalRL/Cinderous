...
describe("RosterAdminModal（ADR-0157：公司設定欄位＋現行名冊預填）", () => {
  ...
  it("ADR-0162：有 relay 檔案上限欄位；以現行名冊政策預填", () => {
    expect(render()).toContain('data-testid="roster-relay-files"');
    const html = render({
      org: "小公司",
      members: [{ pubkey: "a".repeat(64), name: "老闆" }],
      policy: { relayFilesMaxMb: 8 },
      updatedAt: 1,
    });
...
describe("profileGlyph 離職接管（ADR-0163）", () => {
  it("🗄 離職接管優先於其他類型", () => {
    expect(profileGlyph(prof({ orgOffboarded: true }))).toBe("🗄");
    expect(profileGlyph(prof({ orgOffboarded: true, enterprise: true }))).toBe("🗄");
  });
});

describe("RosterAdminModal 公司帳號託管勾選（ADR-0163）", () => {
  it("提供 invite → 有邀請碼＋escrow 勾選（公司帳號）", () => {
    const html = renderToStaticMarkup(
      <I18nProvider locale="zh-Hant">
        <RosterAdminModal
          ...
          onPublish={() => []}
          onCancel={() => {}}
          invite={{ relayUrl: "wss://corp.example", adminPubkey: "a".repeat(64), token: "tok" }}
        />
      </I18nProvider>,
    );
    expect(html).toContain('data-testid="roster-invite-code"');
    expect(html).toContain('data-testid="roster-escrow"');
    expect(html).toContain("公司帳號");
  });
});
