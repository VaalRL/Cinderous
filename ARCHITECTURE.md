...
- **M3**：WebRTC P2P 直連（SDP 信令、Nudge 震動、檔案傳輸）。✅ 核心＋**真實 `RTCPeerConnection`/ICE 已運作**（通話、配對搬家皆走真實 WebRTC）；⏳ **公共 TURN 保底未部署**（ADR-0243）——一般使用者僅 STUN；對稱 NAT／嚴格防火牆下直連失敗**無公共退路**（企業/自架可配 `turnServers`＋`forceTurn`，ADR-0048）。**影響界定：唯通話無退路（M8）；檔案有 relay 退路（ADR-0162）、文字/nudge 本走 relay。**
...
- **M8**：語音/視訊通話——WebRTC media track（P2P）、通話控制信令（kind 21002）、通話 UI。✅（真實 relay/WebRTC E2E）；⏳ **公共 TURN 保底未部署（ADR-0243）**——即時影音**只能**走 P2P 或 TURN、**無文字中繼退路**，故對稱 NAT／嚴格防火牆的兩端**通話接不通**。明確標示、非靜默失敗；公共 TURN 為成本決策（Cloudflare TURN 為候選）。
