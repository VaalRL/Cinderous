# CLAUDE.md（Nostr Buddy 任務路由）

> 本文件是 Nostr Buddy 的任務分配入口。開始任何工作前，先讀 `PRD.md` 與 `ARCHITECTURE.md`，再依任務情境進入 `claude/` 對應指南。
...
Nostr Buddy 的 root 允許存在少量專案入口文件與設定檔：
