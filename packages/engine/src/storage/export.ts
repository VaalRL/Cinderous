...
function gather(storage: AppStorage, opts: ExportOptions): ExportConvo[] {
  ...
  for (const key of keys) {
    const messages = storage.loadMessages(key);
...
export function exportRecords(storage: AppStorage, format: ExportFormat, opts: ExportOptions = {}): string {
  const selfLabel = opts.selfLabel ?? "我";
  const now = opts.now ?? Date.now();
  const convos = gather(storage, opts);
  const nameOf = new Map<string, string>();
