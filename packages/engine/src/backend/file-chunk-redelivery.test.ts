...
// 不是規格。ADR-0273 §1 記錄了它證實的兩個缺陷；**修好之後這裡會紅**，屆時應把斷言
// 改成正確行為（重開不重收、缺塊要報錯），而不是把測試刪掉。
//
// 檔案塊訂閱的重取行為（ADR-0267 §2.2／§2.3 的查證）：那份研究只做靜態閱讀、明載
// 「未寫測試重現」。本檔把它變成事實。
//
...
describe("檔案塊訂閱缺少水位的後果（ADR-0267 §2.3）", () => {
...
describe("截斷後的靜默失敗（ADR-0267 §2.2）", () => {
