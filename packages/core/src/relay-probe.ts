// Relay 行為黑箱探測（ADR-0092；ADR-0275 自 relay/bootstrap 上移至 core）。
//
// 只驗**可觀察的行為**——relay 內部（是否偷記）技術上無法稽核，系統的隱私防線是結構性的
// （E2E／TTL），不依賴信任營運者。
//
// ## 為什麼放 core
//
// 原本只有 CI 的每小時排程在用（`relay/bootstrap/`）。ADR-0275 把它下放給**客戶端**，
// 讓使用者自填的 relay 也能被檢查——而 `relay` 與 `engine` 都依賴 core、彼此不成環，
// 且 core 本就有 WebSocket I/O（`relay-client.ts`），故置於此。實作只用標準 `WebSocket`
// 與本套件的簽章函式，**瀏覽器與 Node 皆可跑**。
//
// ## 探測一律用臨時金鑰
//
// `generateSecretKey()` 每次現產——因此下放到客戶端**不會洩漏使用者身分**，
// relay 只看到一把用完即棄的鑰匙在做行為測試。
//
// ⚠️ 但探測**會寫入事件**（送一顆再查是否還在），故呼叫端必須節制頻率（見 ADR-0275 的快取策略）。

import { buildAuthEvent } from "./nip42.js";
import { generateSecretKey, getPublicKey, type SecretKey } from "./keys.js";
