# Cinderous 前端開發指南（接你自己的 UI）

> 給想用**自己的前端**（Web / React Native / Vue / Svelte / 純 Web Components…）接上 Cinderous 通訊能力的社群開發者。你不需要重寫任何加密、Nostr 協定或 WebRTC——那些已經是**與 UI 無關的共用套件**，你只要蓋 UI。

---

## 1. 心智模型：三層

Cinderous 刻意把「協定/加密」「通訊引擎」「畫面」分成三層。你寫前端＝只碰最上層。
...
├─────────────────────────────────────────────┤
│  @cinderous/core ＋ @cinderous/i18n                  │  ← 直接重用（零 UI 依賴）
...
> 已有實證：`apps/mobile`（react-native-web）就是第二套前端，重用 `@cinderous/core` + `@cinderous/i18n`——證明這套分層真的可換前端。
...
| 層 | 套件 / 位置 | 你要做的事 |
| --- | --- | --- |
| 協定/加密 | `@cinderous/core`（workspace 套件，零 React） | **直接裝來用**，不改 |
| 翻譯 | `@cinderous/i18n`（零依賴，純函式） | **直接用**；要加語言見 §7 |
...
> 📦 **封裝現況**：三層都已是獨立 workspace 套件、可直接依賴——`@cinderous/core`（協定/加密）、`@cinderous/i18n`（翻譯）、**`@cinderous/engine`（通訊後端＋儲存抽象，ADR-0074 K2 已完成）**。你 `import { RelayChatBackend, LocalStorage, type ChatBackend } from "@cinderous/engine"` 即可，不必 fork desktop、不會拖入 React/Tauri。平台特有基質（Tauri 金鑰庫/加密儲存）留在各 app，經 `AppStorage` 介面注入。
...
```ts
import { RelayChatBackend, webSocketConnector, LocalStorage } from "@cinderous/engine";
...
介面定義在 `@cinderous/engine`（`packages/engine/src/backend/types.ts`）。設計成「少數必要、多數可選」，讓你能只實作/消費需要的部分。
...
- `apps/mobile/package.json`：依賴 `@cinderous/core` + `@cinderous/i18n` + `@cinderous/engine` + `react-native-web`，**不依賴 `@cinderous/desktop`**。
- `apps/mobile/src/screens/ContactListScreen.tsx`：用 RN 元件（`View`/`Text`）撰寫，重用 core 的 `npubEncode` 與 i18n 的 `translate`——純呈現層。
- `apps/mobile/src/chat.ts` ＋ `chat.test.ts`：**用 `@cinderous/engine` 的 `BrowserChatBackend` 驅動通訊**——這就是「非桌面前端消費同一套引擎」的活實證（測試會斷言 start 後收到聯絡人）。
...
```ts
import { registerLocale, availableLocales, createT } from "@cinderous/i18n";
...
Cinderous 是 **AGPL-3.0**。這對你的自訂前端意味著：
