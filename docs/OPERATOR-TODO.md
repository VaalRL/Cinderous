...
| **公共 TURN 保底上線** | Cloudflare 開 TURN app → `TURN_KEY_ID`（var）＋`wrangler secret put TURN_API_TOKEN` | ADR-0243、**ADR-0336（前置條件）** | **客戶端＋Worker `/turn` 接線已落地、TDD 綠、未配 secret 即 no-op（純 STUN）**；設好 secret 即全體一般使用者自動享保底，修對稱 NAT／嚴格防火牆下 ~10–20% 通話接不通。設定範例見 `relay/wrangler.toml` 註解。<br>🔵 **2026-08-05 成本已查證，不再是阻擋項**：$0.05/GB 僅計 egress、**每月 1,000 GB 免費**（SFU 與 TURN 共用）⇒ 語音 ~45 MB/h、視訊 ~0.9 GB/h，且只有 fallback 到 TURN 的通話才算 ⇒ 免費額度約 11 萬語音通話小時。<br>🔴 **阻擋項換成濫用曝險（ADR-0336）**：`/turn` 在任何驗證之前，任何人 GET 就拿到有效 **24 小時**的憑證，而 **TURN 是通用中繼**＝免費 proxy；單一 allocation 灌滿 100 Mbps ＝ **$54/天**，且 Cloudflare Realtime **無硬性支出上限**。**下列四項全綠才放 secret**：<br>① `TURN_TTL_SECONDS` 預設 86400→**300**（並驗通話跨越到期不斷線）<br>② `/turn` 綁 relay 連線 token，未帶回 **401**（不是 204——204 語意為「未配置」）<br>③ Cloudflare 帳單通知設 **500 GB**（免費額度 50%，非 100%）<br>④ 裝置層 `allowPublicTurn` 偏好（預設開）＋設定頁取捨文案 |
