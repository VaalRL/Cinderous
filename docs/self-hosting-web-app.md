...
- **CSP 免自己寫**：repo 已附 `apps/desktop/public/_headers`（嚴格 CSP 真標頭），Vite 複製進 `dist`，自動套用。SPA fallback 由 `wrangler.jsonc` 的 `not_found_handling: single-page-application` 處理（不放 `_redirects`——它在 Workers Static Assets 會與 `.html`/`/index` 自動處理衝突、被判無限迴圈）。
...
- **monorepo 注意**：若部署卡在「application detection … run in the root of a workspace」——CF 在 pnpm 工作區根跑偵測失敗。**把專案的「Root directory」設為 `apps/desktop`** 即可（偵測改在單一 app 執行）。repo 已附 `apps/desktop/wrangler.jsonc`（Workers Static Assets：`assets.directory=./dist`＋SPA fallback），走新版 Workers/統一建置流程時 CF 會據此部署；`_headers`（CSP）仍生效、SPA 由 `not_found_handling` 處理。Build 命令仍 `pnpm --filter @cinderous/desktop build`。
