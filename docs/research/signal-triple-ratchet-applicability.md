...
## 5.6 副作用盤點：容量**不是**唯一的代價（2026-09-19）

§5.5 量完成本之後容易得到一個結論：「除了體積變大之外沒有副作用」。**那個結論是錯的。**
逐項查證如下——有四項不是容量問題，其中一項是**必須先做的前置**。

### 🔴 A. 降級偵測目前分辨不出「後量子 → 古典」——**這是前置，不是副作用**

今天的釘選是**布林**（本機查證，`packages/engine/src/storage/types.ts:507`）：

```ts
pinned?: Record<string, boolean>;
```

而降級判定是「我釘過他、但我現在沒有他的 EK」（`relay-backend.ts:1369`）。

⇒ 一個**仍然有 EK、只是退回古典**的對方，在這套判定下**完全看不出來**。
攻擊者（或被控制的中繼）只要供應一顆較舊的 `v1` kind 10040 公告，
我們就會靜默地用古典加密，而使用者看到的仍是「這位聯絡人有 FS」。

🔵 **ADR-0302 早就預見了這件事**——它的標題就是「**釘選要記強度而非布林**」，
而它自己的狀態欄寫著：

> ⏳ **仍未實作**：§3（`pinned` 改記 `{scheme, at}`）、§4 三態 UI 的完整形態、**強度順序表**

⇒ **在做後量子之前必須先把 ADR-0302 §3 做掉**，否則我們會把一個「可被靜默降級」的機制
當成保護在出貨。這一項與容量無關，而且順序不能反。

### 🔴 B. 未經審計——在未審計的基礎上再疊一層

這是 §6.1 的老問題，但值得在這裡重述，因為它**不是容量、而且方向相反**：
後量子做**錯**了會比沒做更糟。

具體的失敗模式不是抽象的：混合式 KDF 若把古典那一半接錯（例如域分離寫錯、
或某個路徑漏掉 `ss_classic`），結果是**古典安全性被降低**，換來一個還沒人需要的後量子性質。
那種錯誤不會有任何測試會紅——兩邊都算得出同一把金鑰，訊息一切正常。

### 🔴 C. 文案風險：簽章仍然是 secp256k1

做完後量子 EK 之後，**訊息的機密性**受到保護，但**身分的真實性**沒有——
`finalizeEvent` 仍是 secp256k1 簽章，量子攻擊者仍然偽造得出事件。

⇒ 「量子安全」這四個字**不能說**。ADR-0306 已經為 FS 立過「文案不得宣稱」的規矩，
後量子需要同一條，而且更容易犯——因為「我們用了 NIST 標準」聽起來很值得寫在官網上。

### 🟠 D. 相依表面積：pre-1.0，且在最敏感的套件裡

`@noble/post-quantum` v0.7.1（2026-08-27）是 **pre-1.0**，要放進 `packages/core`。
ADR-0235 的出貨相依硬閘會涵蓋它，但版本本身的穩定性是另一回事。

🔵 **但「major 版本落差」這條比原稿寫的輕**（本機查證）：樹裡**已經同時存在**
`@noble/curves` 1.9.7 與 2.0.1、`hashes` 1.8.0 與 2.0.1——nostr-tools 用 v2、
我們的 `packages/core` 用 v1，pnpm 一直好好地並存著。
⇒ 加一個要 v2.4 的套件**不會強迫 `packages/core` 升級**，只是多一份副本
（而 §5.5 的 +7.4 KB gzip 已經含那份副本）。

### 🟡 E. 解密失敗的語意（可接受，但要知道）

ML-KEM 用**隱式拒絕**：密文被竄改或金鑰不對時**不會報錯**，而是回一把偽隨機的共享祕密
⇒ 最終在 NIP-44 那層以 MAC 驗證失敗呈現。

⇒ 這剛好**落進 ADR-0316 既有的 `maybeEkLoss` 桶**，不需要新的 UI。
但它也意味著**永遠分不出「後量子那半壞了」與「金鑰不對」**——
與 ADR-0316 檔頭「密碼學上無法區分」那段的誠實是一致的。

### 查過之後**不成立**的顧慮

| 原本擔心 | 查證結果 |
| --- | --- |
| QR 配對裝不下變大的 EK 私鑰（2400 bytes/把） | ❌ 不成立——QR 只帶一次性 AES 金鑰與房間號，捆包走 WebRTC（`packages/core/src/pairing.ts:6`） |
| 相依要 major bump，全部既有密碼學路徑要重驗 | ❌ 不成立——noble v1／v2 已在樹裡並存 |
| CPU 負擔 | ❌ 反過來，比現況的 ECDH 快（§5.5） |
| 撞免費層請求數瓶頸 | ❌ 零影響（§5.5） |

### 小結

> **「除了容量之外沒有副作用」是錯的。**
> 真正的清單是：**先做 ADR-0302 §3**（否則機制可被靜默降級）、**審計風險**、
> **文案不能宣稱量子安全**、**多一個 pre-1.0 的密碼學相依**。
> 容量本身反而是其中最好處理的一項。
...
7. **上游可能有自己的方向**——但查證後**風險比原本寫的低**（2026-09-18 補）。見 §6.1。

## 6.1 「Nostr 上游方向」具體是指什麼（2026-09-18 查證）

上游有三條相關的線，狀態各不相同：

| 線 | 狀態 | 對我們的意義 |
| --- | --- | --- |
| **NIP-44 後量子**（[issue #1971](https://github.com/nostr-protocol/nips/issues/1971)） | **開啟中**，2025-07-10 至今無共識 | 討論方向是「**NIP-44 v3 ＝ 古典 ECDH ＋ ML-KEM 混合**」，與 Apple／Signal／WhatsApp 同一種做法——**與本文 §5 的草案同一個方向** |
| **NIP-104 雙棘輪 DM**（[PR #1206](https://github.com/nostr-protocol/nips/pull/1206)） | 🔴 **已關閉**（2024-06-20） | 見下，對我們是重要旁證 |
| **NIP-EE（MLS）**（[PR #1427](https://github.com/nostr-protocol/nips/pull/1427)） | 進行中 | NIP-104 作者轉向的去處；與 ADR-0091「MLS 暫緩」同一個題目 |

### 🔵 為什麼互通性風險比原本估的低

**我們的後量子改動不會碰到 NIP-44 的線路格式。**

`nostr-tools` 的 API 是 `nip44.encrypt(plaintext, conversationKey)`——**對話金鑰是參數**，
而 `getConversationKey(sk, pk)` 是另一個函式。所以要做後量子，我們改的是
「怎麼算出那把對話金鑰」（`HKDF(ss_classic ‖ ss_pq)`），**payload 仍是 NIP-44 v2 的格式**。

而那一層**正是 EK 已經替換掉的那一層**——EK retarget 做的事就是「把 `pk` 換成別的公鑰」。

更關鍵的是：**啟用 FS 的訊息本來就已經不互通了。** Gift Wrap 加密到 EK 而非身分公鑰，
標準 Nostr 客戶端算出來的對話金鑰是錯的、解不開。而 ADR-0238 §6 的降級機制保證
「對方不支援就退回身分金鑰」⇒ **非 Cinderous 客戶端永遠走古典路徑，不受影響。**

⇒ 後量子 EK 的互通性代價 ≈ **零新增**，因為我們在那條路上已經是獨立的了。
若日後 NIP-44 v3 落地，我們要做的是**改用它的導出方式**，而不是拆掉什麼。
