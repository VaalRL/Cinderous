...
2. **已定案、可直接施工**：**H5 群組快照廣播（ADR-0068）**、**H6 加密備份碼（ADR-0070）**、**Phase I 自動分配/自動搬家（ADR-0069，I1→I4）**——皆 🌐 可驗（I 的實效需錨點營運前提）。
3. **需你決策**：M7 語音訊息離線退回策略、G5 SSO/元資料稽核（先立 ADR）；D4 桌面先行多裝置同步（若要做需立 ADR）。
