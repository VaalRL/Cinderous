...
- **企業模式（Phase G，G0–G3 完成）**：✅ 封閉 allowlist 中繼、單一 App 多身分並存與切換（工作/個人、鎖定/開放、資料命名空間隔離）；✅ 簽章名冊佈建＋企業通訊錄（G1）、政策開關＋**強制 TURN 接入 WebRTC**（G2）、組織群組／公告（G3）。餘 G4 金鑰託管、G5 SSO/元資料稽核。
- **治理**：pnpm monorepo、TS strict、TDD、CI、**51 份 ADR**、AGPL-3.0。✅

**缺口總覽**：Tauri 桌面殼 GUI 接線、relay 生產部署（D1/AUTH）、行動端、企業 G4–G5（金鑰託管、SSO/元資料稽核）、通話 TURN 保底真機驗證、F4 第三方稽核。
...
- **G4 企業金鑰託管**：「換機/遺失還原」需求 vs「設備全毀＝帳號死亡」承諾的取捨（做成選用、加密託管）。實作前須立 ADR。
- **M7 語音訊息離線退回策略**：受中繼大小限制時的退回方式。

（已定案：群組加密方案 → ADR-0027；前向保密 → ADR-0028；@提及 → ADR-0050；對話串 → ADR-0051）

## 建議下一步

此環境（🌐）**不需新決策就能做的規劃項目已全數完成**（Phase A/E 全部、G0–G3、M8 來電鈴聲、Cinder 更名）。往下推進需要：

1. **需你決策**：G4 金鑰託管（先立 ADR 定隱私取捨）。
...
3. **此環境可選打磨**：外撥回鈴音、thread 內 @自動完成、群組成員管理 UI。
