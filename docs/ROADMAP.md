...
| G1 | ① 佈建 + 企業通訊錄 | 📋 **待實作（首推）**：管理者**簽章**組織名冊 → 客戶端自動採用（預填聯絡人、產出 allowlist、離職即撤銷）。**複用 ADR-0035/0039 簽章清單機制**（`verifyRelayList`/`shouldAdoptList`/`maintainerPubkey`）。先立 ADR + core TDD。 |
