...
- **企業模式（Phase G，G0–G3 完成）**：✅ 封閉 allowlist 中繼、單一 App 多身分並存與切換（工作/個人、鎖定/開放、資料命名空間隔離）；✅ 簽章名冊佈建＋企業通訊錄（G1）、政策開關＋強制 TURN 旗標（G2）、組織群組／公告（G3）。餘 G4 金鑰託管、G5 SSO/元資料稽核。
- **治理**：pnpm monorepo、TS strict、TDD、CI、**46 份 ADR**、AGPL-3.0。✅

**缺口總覽**：Tauri 桌面殼 GUI 接線、relay 生產部署（D1/AUTH）、行動端、企業 G4–G5（金鑰託管、SSO/元資料稽核）、@提及/對話串（規劃）。
