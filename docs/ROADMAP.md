...
| F3 | 剩餘 review 技術債 | ✅ **C4 二進位框架**（檔案分塊去 base64 ~33%，ADR-0029）＋ ✅ **A6 ICE candidate 批次**（去抖動合併為單一 `candidates` 信令，減少中繼發佈數；`CandidateBatch` 可測，真實 WebRTC E2E 驗證）。⬜ A5 多設備 sync 上限（進行中）。 |
| F4 | 第三方安全稽核 | 端到端加密與金鑰處理的獨立稽核（README 已聲明尚未稽核）。 |
| F5 | 容量/成本 | 🔧 **心跳合併已實作**：音樂併入心跳（移除 kind 20002 事件與訂閱）+ 送出抖動 jitter（`core` 可測、兩後端套用、示範模式 E2E 驗證 now-playing）；容量模型回填 `docs/adr/0006`。WebRTC 卸載/付費層評估待後續。 |
...
（已定案：群組加密方案 → ADR-0027；前向保密 → ADR-0028）
