...
| H6 | 加密備份碼 | 🌐 | 📋 混合式格式：內層 NIP-49 ncryptsec（nostr-tools）＋外層信封 `{v, ncryptsec, relayUrl}` 成單一字串/QR；設定面板「產生加密備份碼」、SignIn/匯入支援「貼碼＋密碼」還原；密文使用者自持不上雲。明文 nsec 備份降為後備（ADR-0070，部分取代 ADR-0015）。 |
...
| # | 任務 | 環境 | 說明 / 驗證 |
| --- | --- | --- | --- |
| J1 | relay 端快照 kind | 🌐＋☁️ | 📋 可尋址取代語意（`d` tag＝裝置 id、每裝置僅存最新）＋配額強制（單顆 256KB、每 pubkey 5 顆、30 天過期）＋刪除（purge）支援。headless 測試同 C1 模式。 |
| J2 | 客戶端快照發佈 | 🌐 | 📋 三檔內容組裝（基本＝聯絡人/群組/封鎖/設定/已讀；完整＝＋近期訊息）＋NIP-44 加密給自己＋變更觸發防抖＋每日至多一次；關閉時主動刪除雲端快照。 |
| J3 | 新裝置還原合併 | 🌐 | 📋 匯入（nsec／H6 備份碼）後拉取所有裝置快照，以 core `sync.ts` 收斂語意（id 去重＋LWW，交換律）合併；與 7 天收件匣重放共存去重。 |
| J4 | 設定 UI＋政策 | 🌐 | 📋 設定面板「雲端同步」三檔選擇＋狀態（上次備份/大小/裝置數）＋誠實文案（金鑰保護範圍/回溯風險）；模式經 LWW 同步各裝置；企業名冊政策 `disableCloudBackup`（ADR-0048 模式）。 |

**完成定義**：開啟模式下，新裝置憑備份碼＋密碼秒級還原聯絡人/群組/設定（完整模式含近期訊息）；關閉＝雲端零殘留。
...
2. **已定案、可直接施工**：**H5 群組快照廣播（ADR-0068）**、**H6 加密備份碼（ADR-0070）**、**Phase I 自動分配/自動搬家（ADR-0069）**、**Phase J 加密雲端快照（ADR-0071）**——皆 🌐 可驗（I 的實效需錨點營運前提；J1 上線需 `wrangler deploy`）。建議順序：H5 → H6 → J（J3 依賴 H6 的還原入口）→ I。
3. **需你決策**：M7 語音訊息離線退回策略、G5 SSO/元資料稽核（先立 ADR）；D4 桌面先行多裝置同步（完整歷史搬運＋即時通道，若要做需立 ADR）。
