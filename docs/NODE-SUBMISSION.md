...
任何人都能**自架並使用** Cinder 節點（見自架文件，ADR-0075）。本文件是想被**官方自動選座池**收錄
...
2. **暴露自我宣告**：以節點營運者金鑰簽一份 `CinderNodeDeclaration`（`@cinder/core` 的 `signNodeAttestation`），
...
- **只驗行為**：「你有沒有偷記元資料」在你機器裡，**技術上無法稽核**。Cinder 的隱私是**結構性**的
