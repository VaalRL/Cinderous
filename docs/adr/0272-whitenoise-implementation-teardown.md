# 0272. White Noise 實作拆解：他們踩過的坑，有幾個我們形狀相同（研究記錄）
...
> ADR-0271 只讀了 Marmot 規格。本文讀實作。**最有價值的不是他們做對了什麼，
...
⇒ **ADR-0269 §3 說「成熟 MLS 實作在 Rust、TS 沒有同級品」是對的，但漏了下半句：
...
**ADR-0267 查到的「檔案塊 filter 沒有水位、離線私訊 filter 有」不是一個孤立的疏漏，
...
⇒ 只能**分部署**，而且分界線與 ADR-0269 §1 的推播是同一條：
...
⇒ **佐證 ADR-0267 把 NIP-B7 從 Tier 2 升到第一名的判斷**——這不是我們一廂情願的方向，
是這條路上的人已經在走的。
...
- 次之是 §2.1：**ADR-0267 的檔案塊問題應該被理解為「訂閱工作負載混用」的症狀**，
...
- 後續行動（**皆待決策，本 ADR 不執行**）：
  ...
  2. 修 ADR-0267 的檔案塊水位時，順手把「重取策略屬於哪個工作負載」寫成明確結構，
     不要再讓每個 filter 各自記得。
