...
**採選項 1：按收件人 pubkey 前綴分片，並把 presence 拆成獨立層。先取 16 片。** 剩遷移為開放子問題（見下）。
...
> **已定：presence 拆成獨立層**（見〈決策〉）＋ **shard 數先取 16**。餘下**一個**開放子問題（遷移），
> 另記一個待實測的替代方案（一人一片）：

1. **遷移（唯一開放）。** 單一全域 DO 的 SQLite 已存**全體離線留言**；分片後每片各自存。需 dual-write 過渡
   或 cutover——這是本 ADR 最大的**操作風險**。錨點稀少、離線留言 7 天過期，傾向「**切換後讓舊留言自然
   過期**」而非搬遷。
2. **一人一片（選項 1b）＝待實測的替代方案，非現在做。** 血條最好、免片數/遷移，但需先 prototype 量
...
- 後續行動 / 待辦：
  1. ✅ shard 數＝**16**（血條優先、邊際遞減、可低成本再切細）。遷移傾向「切換＋舊留言自然過期」避免搬遷。
