...
- 狀態：提議中（重大架構決策；本 ADR 定方向與開放子問題，經接受後排 prototype／遷移）
- 日期：2026-07-23
...
- **選項 0：維持現狀＋防禦性強化。** 繼續降低崩潰機率（如 C1 的圍籬）。零架構改動，但**血條與天花板
  都在**——單點故障的**影響**不變。
- **選項 1（推薦）：按收件人 pubkey 前綴分片。** `idFromName("shard-" + pubkey前綴)`。收件匣天然同片、
...
**採選項 1：按收件人 pubkey 前綴分片，並把 presence 拆成獨立層。** 本 ADR 定方向；shard 數與遷移為開放子問題（見下）。

- **分片鍵＝收件人 pubkey 前綴。** 例：pubkey 高 nibble ＝ 16 片（血條 1/16、跨分片連線 ≤16）。
...
> **已定方向（原第 3 點 presence 策略）：presence 拆成獨立層**（見〈決策〉）——不進訊息分片，client
> 對它只需一條連線、失效只影響綠點不影響訊息。餘下**兩個**開放子問題：

1. **shard 數（訊息片）。** 16（連線 ≤少數活躍片、血條 1/16、粗）vs 256（血條 1/256、細）。血條優先
   傾向先取**小數**（16），擴充需要時再切細。（presence 拆出後，訊息片的連線只來自「自己＋活躍對話」，
   shard 數不再直接乘 client 連線數。）
2. **遷移。** 單一全域 DO 的 SQLite 已存**全體離線留言**；分片後每片各自存。需 dual-write 過渡或 cutover
   ——這是本 ADR 最大的**操作風險**。錨點稀少、離線留言 7 天過期，或可「切換後讓舊留言自然過期」而非搬遷。
