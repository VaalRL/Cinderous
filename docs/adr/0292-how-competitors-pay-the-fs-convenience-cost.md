# 0292. 對手怎麼處理「前向保密 vs 方便性」——他們沒有免費解掉，是把代價換了位置
...
ADR-0288 查出的檔案塊靜默截斷/重收，正是「沒有保留與續取機制」的症狀。
...
⇒ ~~「per-device 身分」這個方向本身是對的（ADR-0290 §4 已指出我們要走棘輪就得先做這個），~~
🔴 **兩處更正（2026-07-31）**：

1. **主張**（見 ADR-0303 §1／§2）：棘輪的入場費是**每裝置各自的預金鑰 → 各自的 session**，
   **不是** per-device 身分。Marmot 的 per-device MLS leaf 確實**順帶**給了它，
   但那是 MLS 的結構使然，**不代表我們得付同一筆**——我們可以只做 session 那一層。
2. **引用**：`ADR-0290 §4` **是誤引**——0290 §4 講的是元資料，整份 0290 從未提出這個主張。
   該誤引出自本文，並非 0290 的內容；已刪除，不另找替代引用。

**但它不是免費的，而且對手也還沒做完** ——這句仍然成立。

## 4. 附帶發現：他們的推播設計比我在 ADR-0290 §1 建議的形狀更好
...
這比我在 ADR-0290 §1 建議的「中繼在 `#p` 命中時打 APNs」**乾淨**：
那個版本讓中繼多一個角色，這個版本讓推播與中繼解耦。
⇒ **ADR-0290 §1 的建議形狀應據此修訂**（結論「該做推播」不變，做法改）。

## 5. ~~Keychat 未查證~~ → **已由 ADR-0300 查證（2026-07-30）**

> 本節原本寫「沒有讀 Keychat 的原始碼或規格…**不應據此下任何結論**」。
> **保留已解除**，改引 ADR-0300（讀了 `keychat-io/docs`、`keychat-io/keychat-protocol` 的
> spec v2／nip-signal／nip-mls，以及 App 原始碼的備份實作）。三個要點：
>
> - **位址輪替是「每來回」不是「每則訊息」**——規格明文：連發 5 則不換位址。
>   ADR-0289 那句來自行銷語，已更正。
> - **多設備他們也沒解**。`multi-identities.md` 內容為空；`keychat-protocol` 的
>   multi-identity 是「一個 runtime 多個 Nostr 身分」（＝我們 ADR-0138 的多身分），
>   不是多設備。他們重用 libsignal 但**沒有重用 Sesame**。
> - **最重要的一項**：他們的加密備份檔是「Argon2id(密碼) → AES-256」，內容是
>   **目錄下所有 DB（含 ratchet 狀態）＋ `secure_storage.json`（助記詞就在裡面）**
>   ⇒ 一個檔案一個密碼包住身分＋全部歷史＋會話狀態。
>   **傳輸側 FS 再強也保護不到它**——這支持 ADR-0298 的前提。
...
- 本文**不改變任何產品行為**。
- 對 ADR-0291 的補充：方便性代價**不是我們獨有的怯懦**，是這條路上所有人都在付的。
...
- 後續行動（**皆待決策**）：
  1. 修訂 ADR-0290 §1 的推播建議形狀（改為「通知伺服器與中繼解耦、寄件者觸發」）。
  2. 若要補「亂序/遲到的保留與續取」能力，先用在 ADR-0288 的檔案塊問題上——
     那裡今天就在痛，且不需要任何密碼學變更。
