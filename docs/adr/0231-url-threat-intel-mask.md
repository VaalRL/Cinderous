...
- **實作階段**：
  - **P1（已落地）**：core `threat-intel.ts`（4 測試）＋ desktop `assessUrl` 延伸（`known-malicious`＋`sources`，23 測試）＋ i18n `urlrisk_knownMalicious`。
  - **P2**：來源抽取 build script → snapshot → 官網部署＋CI 定期重建；app 拉取（opt-in、節流、失敗靜默）。
  - **P3**：desktop 遮罩 UI（遮住＋來源＋一般可展開/嚴格不可展開）＋送出端警示＋嚴格模式＋設定四項＋i18n。
  - **P4**：官網介紹（純本地、不送 URL、可自訂可關）。
