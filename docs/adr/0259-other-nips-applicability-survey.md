# 0259. 其他 NIP 的適用性盤點（研究記錄）

- 狀態：**研究記錄（未決策，本文不改變任何產品行為）**
  ／**已由 ADR-0288 補正**：本文自稱盤點「全部 NIP」但漏了 11 個，且對 NIP-67 的排序有誤；
  Tier 1 三項已由 ADR-0260 實作完畢。
- 日期：2026-07-27
...
- 營運者贊助入口（ADR-0089）——目前**完全沒有**收入面（見 ADR-0257 §2）；
...
對**企業／自架客戶**（ADR-0257 的目標客群）差別很大——他們可以把 blob 放進自己機房的 Blossom
...
**部署期佈建**（env／匯出名冊）。ADR-0257 §4.2 把「託管營運」列為主要收入項，而託管必然需要
