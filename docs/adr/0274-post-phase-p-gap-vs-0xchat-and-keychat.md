# 0274. 修完 Phase P 之後，我們與 0xchat／Keychat 還差什麼（研究記錄）
...
- 資料來源：**clone `keychat-io/keychat-app` 與 `0xchat-app/0xchat-core` 讀其 README 與設計文件**
  （2026-07-28）——ADR-0271 §5 曾明載「Keychat 完全未查證」，本文補上

> 一句話結論：**修完 Phase P 幾乎不會改變任何競爭差異。**
> P1–P4 修的是**只有我們有的 bug**（對手有沒有等價 bug 我沒查），P5 是唯一會動到競爭軸的一項，
> 而它也只補到一半。真正的差異在別處，且大多是**刻意選的**。

## 0. ⚠ 先更正 ADR-0268 的一個錯誤主張

ADR-0268 §2.5 寫：「本專案已有而 0xchat **尚未有**的：WebRTC 語音／視訊通話（0xchat 列為規劃中）」。
...
要補完另一側得讓 `#p` 也輪替，而 ADR-0269 §4 已判定不追（會改寫收件匣模型，
...
（ADR-0271 §2）。**但我們沒上線**（Phase 3 審計未過），他們的至少是 Beta 可用。
...
1. **群組 FS／PCS**——White Noise 有、我們沒有（ADR-0269 §3；本文兩個對手中 0xchat 的 FS 只在 1:1 的 Secret Chat）。
2. **1:1 的 FS 強度與元資料**——Keychat 的 per-message 棘輪＋地址輪替嚴格更強；
   P5 只補發布側，接收側的 `#p` 仍穩定。
3. **1:1 FS 尚未可產線啟用**——卡外部審計（ADR-0269 §2），對手至少 Beta 可用。
4. **行動端沒有推播**——ADR-0269 §1；Phase P 不含此項（它卡在真 RN 移植）。
5. **零生態網路效應**——刻意，但代價真實。
...
- **Phase P 該做，但不要當成追趕**——它是把自己的地板補平（P1/P2 是已測證實的缺陷）。
  真正影響競爭的是 ADR-0269 的優先序（推播 → FS 審計 → 入站互通 → 群組粗粒度 FS）。
- **§3.2 值得單獨記住**：在「FS 是否犧牲歷史」這一格，我們的設計比兩個對手都好——
  這是可以講的差異，前提是**先把它上線**。

## 後果

- 正面：更正 ADR-0268 一個站不住的競爭主張；把 Keychat 從「完全未查證」變成有據；
